...
> Note: Lucien de Rubempré n’apparaît pas réellement dans 6 romans différents: Jeon et Lee ont compté comme des romans distincts chacune des trois parties d*’Illusions perdues*, parues d’abord, il est vrai, comme des romans autonomes entre 1837 et 1843, puis réunis en 1843 sous le titre *Illusions perdues* dans le tome VIII de *La Comédie humaine*, dans les *Scènes de la vie de province*.

Pour les liens, nous utiliserons la cooccurrence des personnages au sein d’un même roman. Par exemple, si Lucien et Nathan apparaissent dans un roman, un lien de valeur 1 sera compté.

> Question: quelles conséquences découlent de ce choix? Quelle simplification ce choix suppose-t-il? Rappelez-vous des limitations évoquées par Franco Moretti dans son analyse de *Hamlet*.
...
Cette table des liens est redondante. Par exemple, si Rastignac et Vautrin apparaissent tous deux dans trois romans, nous avons trois lignes pour le lien « Rastignac-Vautrin ». Ce que nous voulons, c’est un seul lien entre eux, mais un lien proportionnel au nombre d’occurrences. Nous allons donc compter ces occurrences pour en faire le poids du lien. Enfin, nous allons extraire la liste unique des personnages pour créer notre table des sommets.
...
Dans notre premier exercice, nous avons adopté une vue **macro**. Nous avons survolé *La Comédie humaine* pour cartographier les grandes constellations de personnages qui traversent l’œuvre. Pour cela, nous avons utilisé un jeu de données déjà structuré où le lien était simple : deux personnages étaient liés s’ils apparaissaient dans le même roman.
...
Pour cette analyse, nous devons aussi redéfinir ce qu’est un « lien ». Si le lien n’est plus la coprésence dans un roman, quelle est sa nouvelle définition ? Nous ferons un choix méthodologique courant : **deux personnages seront considérés comme liés s’ils sont mentionnés dans le même paragraphe**. Le paragraphe représente une unité de scène ou de pensée cohérente, rendant leur coprésence significative. Précisions qu’il s’agit d’un choix pédagogique et pratique orientée vers l’exploration plus que la démonstration.

Notre projet sera donc de «lire» avec R le texte des *Illusions perdues*, d’y détecter les personnages grâce à notre dictionnaire, de construire le réseau de leurs interactions paragraphe par paragraphe et, enfin, d’exporter le tout pour l’analyser dans Gephi.
...
Nous chargeons maintenant dans l’environnement de programmation le dictionnaire des personnages d’*Illusions perdues* que j’ai fabriqué.
...
Comme nous l’avons dit déjà, un même personnage peut être désigné de plusieurs façons dans un roman : par son prénom, son nom de famille, un titre, un surnom, etc. Pour que l’analyse de réseau soit fiable, il faut idéalement regrouper toutes ces formes sous un **nom canonique unique**.
...
3.  Vérifier que chaque variante est bien associée
...
>
> Cette table de correspondance permet de détecter les personnages dans le texte, même lorsqu’ils sont mentionnés sous des formes variées. Elle est essentielle pour éviter les doublons et garantir la cohérence du réseau.
...
## Pourquoi segmenter le texte ?

La segmentation permet de diviser le texte en **unités d’analyse** homogènes. Chaque segment est une fenêtre d’observation dans laquelle on peut détecter la présence simultanée de plusieurs personnages.

Deux paramètres contrôlent cette opération :

\- **Longueur du segment** : nombre de mots par segment

\- **Chevauchement** : proportion de mots partagés entre deux segments consécutifs
...
# Maintenant, toutes les méthodes fonctionneront sur l’intégralité du texte.
...
# Méthode 3 : Par mots - Cette fois, `segments_mot` contiendra de nombreux éléments.
segments_mot <- segmenter_texte(corpus_texte_integral, methode = "mot", taille_segment = 100, chevauchement = 20)
cat("--- Segmentation par 100 mots (chevauchement de 20) ---\n")
cat("Segment 1 (mots 1-100) :\n", substr(segments_mot[[1]], 1, 300), "...\n\n")
cat("Segment 2 (mots 81-180) :\n", substr(segments_mot[[2]], 1, 300), "...\n")
```

Dans les prochaines étapes, nous utiliserons ces segments pour détecter les personnages présents et construire la matrice de cooccurrence.
